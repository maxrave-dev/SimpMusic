...
internal class DataStoreManagerImpl(
    ...
    override val streamProxyFallback: Flow<String> =
        settingsDataStore.data.map { preferences ->
            preferences[STREAM_PROXY_FALLBACK] ?: TRUE
        }

    override suspend fun setStreamProxyFallback(enabled: Boolean) {
        withContext(Dispatchers.IO) {
            settingsDataStore.edit { settings ->
                settings[STREAM_PROXY_FALLBACK] = if (enabled) TRUE else FALSE
            }
        }
    }
    ...
    companion object Settings {
        ...
        val STREAM_PROXY_FALLBACK = stringPreferencesKey("stream_proxy_fallback")
