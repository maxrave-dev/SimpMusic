...
import com.maxrave.kotlinytmusicscraper.models.YouTubeClient.Companion.MWEB
...
class YouTube(
    ...
    private val isWatch: Boolean = context.packageManager.hasSystemFeature(PackageManager.FEATURE_WATCH)

    private val ytMusic = Ytmusic(context)
    private val nSigCache = ConcurrentHashMap<String, String>()
    private val preflightDecisionCache = ConcurrentHashMap<String, Pair<Long, Boolean>>()

    // A small "does this URL actually work?" probe to avoid returning URLs that 403 immediately.
    // One tiny range request per tested InnerTube client.
    private val googleVideoPreflightClient: OkHttpClient by lazy {
        OkHttpClient
            .Builder()
    ...
    @Volatile
    private var proxyFallbackEnabled: Boolean = true

    fun setProxyFallbackEnabled(enabled: Boolean) {
        proxyFallbackEnabled = enabled
    }
    ...
    suspend fun smartTubePlayer(
        ...
        Logger.d(TAG, "YouTube TempRes ${tempRes.playabilityStatus}")
        // Even when the InnerTube player response is UNPLAYABLE, external extractors may still
        // be able to provide usable stream URLs. Prefer "does it have streamingData we can patch"
        // over the playability status.
        sigResponse = tempRes
        if (sigResponse.streamingData == null) return null
                ...
                streamingData =
                    sigResponse.streamingData?.copy(
                        formats =
                            sigResponse.streamingData.formats?.map { format ->
                                format.copy(
                                    url = streamsList.find { it.iTag == format.itag }?.url,
                                )
                            },
                        adaptiveFormats =
                            sigResponse.streamingData.adaptiveFormats.map { adaptiveFormats ->
                                adaptiveFormats.copy(
                                    url = streamsList.find { it.iTag == adaptiveFormats.itag }?.url,
                                )
                            },
                    ),
            )
    ...
    private data class YouTubeHttpHeaders(
        val userAgent: String,
        val origin: String? = null,
        val referer: String? = null,
    )

    private fun headersForClient(clientId: String?): YouTubeHttpHeaders {
    ...
    private fun preferredPlayerClients(): List<YouTubeClient> =
        if (isWatch) {
            // Wear strict mode: keep stream signing deterministic and avoid cross-client URL churn.
            listOf(ANDROID_VR)
        } else {
            // InnerTune-style first chain, followed by web fallbacks.
            listOf(ANDROID_MUSIC, IOS, TVHTML5, WEB_REMIX, WEB, MWEB)
        }.distinctBy { it.clientName }

    private fun toProbeCandidates(response: PlayerResponse?): List<ProbeCandidate> {
        val streamingData = response?.streamingData ?: return emptyList()
        val audioPriorityItags = listOf(251, 250, 249, 140, 139, 141)
        val adaptiveAudio = streamingData.adaptiveFormats.filter { it.isAudio && !it.url.isNullOrBlank() }
        val muxedAudio = streamingData.formats?.filter { it.isAudio && !it.url.isNullOrBlank() }.orEmpty()
        val videoFallback =
            (streamingData.formats.orEmpty() + streamingData.adaptiveFormats)
                .filter { !it.isAudio && !it.url.isNullOrBlank() }

        val orderedAudioByItag =
            audioPriorityItags.mapNotNull { target ->
                (adaptiveAudio + muxedAudio).firstOrNull { it.itag == target }
            }
        val ordered =
            orderedAudioByItag +
                adaptiveAudio.sortedBy { it.bitrate ?: Int.MAX_VALUE } +
                muxedAudio.sortedBy { it.bitrate ?: Int.MAX_VALUE } +
                videoFallback.sortedBy { it.bitrate ?: Int.MAX_VALUE }

        return ordered
            .mapNotNull { format ->
                format.url?.let { url -> ProbeCandidate(format.itag, url) }
            }.distinctBy { it.url }
    }

    private fun pickPlayableCandidate(
        mediaId: String,
        candidates: List<ProbeCandidate>,
        maxChecks: Int,
        sourceClientName: String? = null,
        stageLabel: String = "direct",
    ): ProbeCandidate? {
            ...
            candidates
                .mapIndexed { index, candidate ->
                    Triple(
                        index,
                        candidate,
                        StreamHealthRegistry.candidatePenalty(mediaId, candidate.itag, candidate.url),
                    )
                }.sortedWith(compareBy<Triple<Int, ProbeCandidate, Int>> { it.third }.thenBy { it.first })
                .map { it.second }

        for ((index, candidate) in rankedCandidates.withIndex()) {
            StreamHealthRegistry.rememberCandidate(mediaId, candidate.itag, candidate.url)
            val candidateClient = StreamHealthRegistry.urlClient(candidate.url) ?: sourceClientName
            val enforceBlock = !(isWatch && candidateClient.equals(ANDROID_VR.clientName, ignoreCase = true))
            if (enforceBlock && StreamHealthRegistry.isClientBlocked(mediaId, candidateClient)) {
                Logger.w(
                    TAG,
                    "Skip blocked client mediaId=$mediaId stage=$stageLabel client=$candidateClient itag=${candidate.itag}",
                )
                continue
            }
            if (StreamHealthRegistry.isUrlBlocked(candidate.url)) {
                continue
            }
            if (index >= maxChecks) {
                return candidate
            }
            if (preflightGoogleVideoOrTrue(candidate.url, mediaId, sourceClientName)) {
                StreamHealthRegistry.markUrlHealthy(candidate.url)
                return candidate
            }
            StreamHealthRegistry.markUrlFailure(candidate.url)
            if (enforceBlock) {
                StreamHealthRegistry.markClientFailure(mediaId, candidateClient)
            }
            Logger.w(
                TAG,
                "Reject candidate mediaId=$mediaId stage=$stageLabel client=$candidateClient c=${StreamHealthRegistry.urlClient(candidate.url)} itag=${candidate.itag}",
            )
        }
        return rankedCandidates
            .firstOrNull { candidate ->
                val clientName = StreamHealthRegistry.urlClient(candidate.url) ?: sourceClientName
                (isWatch && clientName.equals(ANDROID_VR.clientName, ignoreCase = true)) ||
                    !StreamHealthRegistry.isClientBlocked(mediaId, clientName)
            }?.also { StreamHealthRegistry.markUrlHealthy(it.url) }
    }

    private suspend fun resolveViaProxyFallback(
        videoId: String,
        seedResponse: PlayerResponse,
        maxChecks: Int,
    ): PlayerResponse? {
        for (instance in listPipedInstances) {
            runCatching {
                val piped = ytMusic.pipedStreams(videoId, instance).body<PipedResponse>()
                val stream = piped.audioStreams + piped.videoStreams
                if (stream.isEmpty()) return@runCatching null
                val pipedPlayerResponse =
                    seedResponse.copy(
                        streamingData =
                        ...
                        videoDetails = seedResponse.videoDetails?.copy(),
                        playbackTracking = seedResponse.playbackTracking,
                    )
                val playable =
                    pickPlayableCandidate(
                        mediaId = videoId,
                        candidates = toProbeCandidates(pipedPlayerResponse),
                        maxChecks = maxChecks,
                        sourceClientName = "PROXY",
                        stageLabel = "proxy",
                    )
                if (playable != null) {
                    Logger.w(TAG, "Proxy fallback resolved mediaId=$videoId instance=$instance")
                    return pipedPlayerResponse
                }
                null
            }.onFailure {
                Logger.w(TAG, "Proxy fallback failed mediaId=$videoId instance=$instance reason=${it.message}")
            }
        }
        return null
    }
    ...
    suspend fun player(
        ...
        runCatching {
            ...
            val listClients = preferredPlayerClients()

            var decodedSigResponse: PlayerResponse? = null
            var resolvedClientName: String? = null
            var resolvedProviderName: String? = null
            var proxySeedResponse: PlayerResponse? = null
            clientLoop@ for (client in listClients) {
                val enforceBlock = !(isWatch && client.clientName.equals(ANDROID_VR.clientName, ignoreCase = true))
                if (enforceBlock && StreamHealthRegistry.isClientBlocked(videoId, client.clientName)) {
                    Logger.w(TAG, "Skip blocked client mediaId=$videoId client=${client.clientName}")
                    continue
                }
                val tempRes =
                    try {
...
//                                poToken = getWebClientPoTokenOrNull(videoId)?.streamingDataPoToken,
                        ...
                        if (!isWatch) {
                            Logger.d(TAG, "YouTube Player request failed with client ${client.clientName}: ${e.message}")
                        }
                        if (enforceBlock) {
                            StreamHealthRegistry.markClientFailure(videoId, client.clientName)
                        }
                        continue
                    }
                proxySeedResponse = tempRes

                val providerResponses =
                    buildList<Pair<String, PlayerResponse>> {
                        decodeCipherStreamingDataOrNull(tempRes)?.let { add("cipher" to it) }
                        newPipePlayer(videoId, tempRes)?.let { add("newpipe" to it) }
                        smartTubePlayer(videoId, tempRes)?.let { add("smarttube" to it) }
                        if (shouldYtdlp || !isWatch) {
                            ytDlpPlayer(videoId, tempRes)?.let { add("ytdlp" to it) }
                        }
                    }

                for ((providerName, response) in providerResponses) {
                    val probeCandidates = toProbeCandidates(response)
                    val playableCandidate =
                        pickPlayableCandidate(
                            mediaId = videoId,
                            candidates = probeCandidates,
                            maxChecks = if (isWatch) 7 else 9,
                            sourceClientName = client.clientName,
                            stageLabel = "direct:$providerName",
                        )
                ...
                if (!isWatch) {
                    Logger.d(TAG, "YouTube Player no playable URL with client ${client.clientName}")
                }
            }

            if (decodedSigResponse == null && proxyFallbackEnabled && proxySeedResponse != null) {
                resolveViaProxyFallback(
                    videoId = videoId,
                    seedResponse = proxySeedResponse,
                    maxChecks = if (isWatch) 5 else 7,
                )?.let { proxyResponse ->
                    decodedSigResponse = proxyResponse
                    resolvedClientName = "PROXY"
                    resolvedProviderName = "piped"
                }
            }

            if (decodedSigResponse != null) {
                val resolved =
                    "YouTube Player resolved via client=$resolvedClientName provider=$resolvedProviderName"
                if (isWatch) {
                    Logger.w(TAG, resolved)
                } else {
                    Logger.d(TAG, resolved)
                }
            }
            if (decodedSigResponse == null) {
                val noLoginClient = if (isWatch) ANDROID_VR else IOS
                val (tempCookie, visitorData, playbackTracking) = getVisitorData(videoId, playlistId)
                val now = System.currentTimeMillis()
                val poToken =
                    if (now < poTokenObject.second) {
		                if (!isWatch) Logger.d(TAG, "Use saved PoToken")
	                        poTokenObject.first
	                    } else {
                        ytMusic
		                ...
		                if (!isWatch) Logger.d(TAG, "PoToken $poToken")
		                val rawPlayerResponse =
                            ytMusic.noLogInPlayer(
                                client = noLoginClient,
                                videoId = videoId,
                                cookie = tempCookie,
                                visitorData = visitorData,
                                poToken = poToken ?: "",
                            ).body<PlayerResponse>()
		                val playerResponse = decodeCipherStreamingDataOrNull(rawPlayerResponse) ?: rawPlayerResponse
		                if (!isWatch) {
                ...
                val firstThumb =
	                ...
	                if (!isWatch) Logger.d(TAG, "Player Response formatList $formatList")
	                val adaptiveFormatsList = playerResponse.streamingData?.adaptiveFormats?.map { Pair(it.itag, it.isAudio) }
	                if (!isWatch) Logger.d(TAG, "Player Response adaptiveFormat $adaptiveFormatsList")
                val playableCandidate =
                    pickPlayableCandidate(
                        mediaId = videoId,
                        candidates = toProbeCandidates(playerResponse),
                        maxChecks = if (isWatch) 6 else 8,
                        sourceClientName = noLoginClient.clientName,
                        stageLabel = "nologin",
                    )
	                if (!isWatch) Logger.d(TAG, "Player Response playableUrl ${playableCandidate?.url}")

                if (playerResponse.playabilityStatus.status == "OK" &&
                    (formatList != null || adaptiveFormatsList != null) &&
                    playableCandidate != null
	                ) {
                    return@runCatching Triple(
                        ...
                        if (proxyFallbackEnabled) {
                            resolveViaProxyFallback(
                                ...
                                maxChecks = if (isWatch) 4 else 6,
                            )?.let { pipedPlayerResponse ->
                                return@runCatching Triple(
                                    null,
                                    pipedPlayerResponse,
                                    thumbnails,
    ...
    /**
     * Lightweight "does this URL work?" probe.
     *
     * We only probe `googlevideo.com/videoplayback` URLs since those are the ones that often 403
     * due to throttling/signature validation changes.
     */
    fun preflightGoogleVideoOrTrue(
        url: String,
        mediaId: String? = null,
        sourceClientName: String? = null,
    ): Boolean {
        // Most problematic URLs are googlevideo videoplayback; for other hosts, skip preflight.
        if (!url.contains("googlevideo.com", ignoreCase = true)) return true
        if (StreamHealthRegistry.isUrlBlocked(url)) return false
        ...
        val fixed = fixThrottlingParamOrSelf(url)
        val decision =
            runCatching {
            val req =
                Request
                    ...
                    .build()
            googleVideoPreflightClient.newCall(req).execute().use { resp ->
                when (resp.code) {
                    200, 206 -> true
                    403, 404, 410 -> {
                        val candidateClient = StreamHealthRegistry.urlClient(url) ?: sourceClientName
                        val enforceBlock = !(isWatch && candidateClient.equals(ANDROID_VR.clientName, ignoreCase = true))
                        Logger.w(
                            TAG,
                            "Preflight reject mediaId=${mediaId ?: "unknown"} code=${resp.code} client=$candidateClient c=${StreamHealthRegistry.urlClient(url)}",
                        )
                        if (!mediaId.isNullOrBlank() && enforceBlock) {
                            StreamHealthRegistry.markClientFailure(mediaId, candidateClient)
                        }
                        false
                    }
            ...
            Logger.d(TAG, "Preflight inconclusive: ${t.javaClass.simpleName} ${t.message}")
            true
        }

        val positiveTtlMs = if (isWatch) 8_000L else 45_000L
        val negativeTtlMs = if (isWatch) 4_000L else 10_000L
        preflightDecisionCache[url] = (now + if (decision) positiveTtlMs else negativeTtlMs) to decision
        if (decision) {
...
//            YoutubeJavaScriptPlayerManager.getUrlWithThrottlingParameterDeobfuscated(videoId, url.toString())
            ...
            val fixedThrottling =
                nSigCache[nSigParam]
                    ?: mAppService.extractNSig(nSigParam).also { nSigCache[nSigParam] = it }
    ...
    private fun fixThrottlingParamOrSelf(urlString: String): String {
        return runCatching {
            val url = URLBuilder(urlString)
            val n = url.parameters["n"] ?: return@runCatching urlString

            val fixed =
                nSigCache[n]
                    ?: runCatching { mAppService.extractNSig(n) }
                        .getOrNull()
                    ?.takeIf { it.isNotBlank() }
                    ?.also { nSigCache[n] = it }
                    ?: return@runCatching urlString

            if (fixed == n) return@runCatching urlString

            // Replace only the `n` parameter; do not mutate any other signature-bearing params.
            val out = URLBuilder(urlString)
            out.parameters["n"] = fixed
            out.toString()
        }.getOrDefault(urlString)
    }

    private suspend fun decodeCipherStreamingDataOrNull(tempRes: PlayerResponse): PlayerResponse? {
        val sd = tempRes.streamingData ?: return null

        val decodedFormats =
            sd.formats?.map { format ->
                if (!format.url.isNullOrBlank()) return@map format.copy(url = fixThrottlingParamOrSelf(format.url))
                val cipher = format.signatureCipher ?: return@map format
                val decoded = decodeSignatureCipher(cipher) ?: return@map format
                format.copy(url = decoded)
            }

        val decodedAdaptive =
            sd.adaptiveFormats.map { format ->
                if (!format.url.isNullOrBlank()) return@map format.copy(url = fixThrottlingParamOrSelf(format.url))
                val cipher = format.signatureCipher ?: return@map format
                val decoded = decodeSignatureCipher(cipher) ?: return@map format
                format.copy(url = decoded)
            ...
            tempRes.copy(
                streamingData =
                    sd.copy(
                        ...
                        adaptiveFormats = decodedAdaptive,
                    ),
            )

        val candidate =
            (decodedFormats.orEmpty() + decodedAdaptive)
    ...
    suspend fun visitorData(): String? =
        try {
            Json
                ...
                .jsonArray[2]
                .jsonArray
                .firstOrNull { (it as? JsonPrimitive)?.content?.startsWith(VISITOR_DATA_PREFIX) == true }
                ?.jsonPrimitive
                ?.content
        } catch (e: Exception) {
            // Missing visitorData is not fatal; avoid throwing/log-spamming (especially on WearOS).
            null
        }
