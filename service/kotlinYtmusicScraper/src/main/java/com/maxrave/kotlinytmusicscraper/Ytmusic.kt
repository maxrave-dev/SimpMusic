...
class Ytmusic(
    ...
    private val isWatchDevice: Boolean by lazy(LazyThreadSafetyMode.NONE) {
        // Cheap runtime check; avoids tying this shared module to a Wear-specific build variant.
        context.packageManager?.hasSystemFeature(PackageManager.FEATURE_WATCH) == true
    }

    private val ytDlp =
        try {
            // youtubedl-android is heavy and frequently broken on WearOS (missing native assets).
            // Skipping it on watches improves startup time and avoids noisy stacktraces.
            if (isWatchDevice) {
                null
            } else {
    ...
    internal fun updateYtdlp() {
        // On some devices (notably WearOS) youtubedl-android fails to initialize due to missing
        // native assets. Avoid throwing when update is attempted from shared init code.
        if (ytDlp == null) return
        runCatching {
            YoutubeDL.getInstance().updateYoutubeDL(context)
        }.onFailure {
            Logger.e(TAG, "failed to update youtubedl-android", it)
        }
    }
    ...
    private fun createClient() =
        HttpClient(OkHttp) {
            ...
            engine {
                config {
                    // WearOS can be noticeably slower (radio wakeups, BT proxying), so give requests more headroom.
                    val connectSec = if (isWatchDevice) 20L else 15L
                    val readSec = if (isWatchDevice) 60L else 30L
                    val writeSec = if (isWatchDevice) 60L else 30L
                    val callSec = if (isWatchDevice) 90L else 60L
...
//            }
            install(Logging) {
                ...
                // LogLevel.ALL is extremely noisy and can noticeably degrade performance on WearOS.
                // Keep logs minimal there; on phones, keep the current verbosity.
                level = if (isWatchDevice) LogLevel.NONE else LogLevel.ALL
            }
...
//    --data-raw '["O43z0dpjhgX20SCx4KAo"]'
    ...
    suspend fun noLogInPlayer(
        ...
        header("X-YouTube-Client-Name", "${client.xClientName ?: 1}")
        header("X-YouTube-Client-Version", client.clientVersion)
        setBody(
            PlayerBody(
                context = client.toContext(locale, visitorData ?: this@Ytmusic.visitorData),
                playlistId = null,
                cpn = null,
                videoId = videoId,
    ...
    suspend fun is403Url(url: String): Boolean {
        // Best-effort check using a tiny GET range. HEAD can succeed while ranged media fetches
        // still fail with 403 on some googlevideo endpoints.
        return try {
            httpClient
                .get(url) {
                    headers {
        ...
        } catch (_: io.ktor.client.plugins.RedirectResponseException) {
            false
        } catch (e: io.ktor.client.plugins.ClientRequestException) {
            e.response.status.value == 403
        } catch (_: io.ktor.client.plugins.ServerResponseException) {
            false
        } catch (_: Exception) {
            false
        }
