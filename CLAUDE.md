...
- **loginSync/**: the QR-paired transport that moves sign-ins from the Android app to the Desktop app (Ktor client on the phone, one-shot Ktor CIO server on the Desktop, AES-256-GCM). Android + JVM targets only; carries opaque bytes — the data layer decides what they mean
...
- **Radio queues can be kept audio-only (2026-08-16, issue #2334)**: an opt-in setting (**off** by default) that drops video entries from radio queues only — deliberately not a global "hide all video" switch, so a playlist or album the user picked still plays what it contains. Filtering lives in `SongRepositoryImpl` because that is where the three queue sources meet: `getRadioFromEndpoint`, `getRelatedData` and `getContinueTrack`.
  - **Substitution is impossible, so entries are dropped.** Every video that actually reached the measured radio was `MUSIC_VIDEO_TYPE_UGC` — a fan remix or mashup that exists only as a video and ships **no `counterpart`** to swap in. Official music videos never arrive as the primary rendition at all: YouTube already demotes those to the counterpart of the audio track. Rate is roughly 1 entry per 50, so the queue barely shortens.
    - *Superseded 2026-09-25* — that measurement was a radio started from a **song**. A radio started from a **video** is nothing but videos (a UGC seed: 38 OMV, 11 UGC, 1 podcast episode, no song), so dropping every video emptied the queue outright. The setting is now **"Play audio version instead of MV in radio"**, the same idea as YouTube Music's own *Don't play music videos*: a video row whose song rendition YouTube shipped alongside it (`SongItem.counterpart`, from `NextPage.fromPlaylistPanelContent`, logged-in responses only) is swapped for that song; a video with no song version is skipped as before; and a page left empty is kept as it came, since a radio that plays videos beats one that plays nothing. `preferAudioWhenRadioAudioOnly` logs rows / swapped / kept per page — whether a logged-in video radio really carries song counterparts was not measured, only inferred from the song-radio wrappers.
...
- **The header gate is orientation, not platform (2026-08-17)**: the migration above shipped with `val isMobilePortrait = true` hardcoded in all four screens — a "temporarily forced open so the layout can be judged on Desktop" that also **replaced the body of the portrait branch** with the landscape header. Android portrait therefore rendered the desktop header, and Android landscape/tablet lost the old layout entirely. The gate is now `val isPortrait = screenInfo.wDP < screenInfo.hDP` and nothing else: a portrait window (phone upright, or a narrow desktop window) gets the edge-to-edge artwork header, a landscape one gets the side-by-side header. `getPlatform()` no longer appears in Album/Playlist — the one platform check left in this family is ArtistScreen's `HazeProgressive` guard, which is about skiko, not about layout.
  ...
  - Artist keeps three values on the orientation: frame `aspectRatio(1f)` vs `height(hDP / 2)`, `ContentScale.FillWidth` vs `Crop`, scrim `wDP * 0.7f` vs `hDP * 0.35f` — both figures are 70% of the frame's *own* height, which is why they differ.
    - *Superseded 2026-09-21*, to match Apple Music. The landscape frame now takes the banner's own shape at full width, `aspectRatio(url.thumbnailAspectRatio())`, and pushes the page down.
      - The ratio is read off the URL's `w`/`h` spec, so the frame is right before the image loads.
      - It falls back to `height(hDP / 2)` when the image is narrower than 1.5:1, since a square photo at full width would swallow the page.
      - The scrim is now `fillMaxHeight(0.7f)` of the frame, whatever its shape.
...
- **System equalizer returns as an option (2026-09-13)**: Android gets an *Equalizer type* choice in Settings → Audio — `equalizer_type` in DataStore, `BUILT_IN` (default) or `SYSTEM` — and the two never run together. The system side is the pre-2.0.0 code restored verbatim (app `f7f35e96^`, core `7ddeb379`, identical to v1.7.0): `MediaPlayerListener.shouldOpenOrCloseEqualizerIntent`; in the adapter, the `notifyEqualizerIntent` calls (OPEN when the current player moves into `PLAYING`; CLOSE on pause, stop, end of queue and cast handoff) **and** the per-player `onEvents` hook, which re-sends OPEN or CLOSE on every playback-state, play-when-ready, is-playing or position-discontinuity event of the current player (restored in the never-constructed `ExoPlayerAdapter` too, for parity); the handler's `sendOpen/CloseEqualizerIntent` with CLOSE in `release()`; the `OpenEq` expect/actual pair and the *Open system equalizer* row. The only new logic is the gate: with `BUILT_IN` the handler drops every open/close request; with `SYSTEM` it flattens the built-in curve and lets them through; switching while music plays sends CLOSE or OPEN at once, because nothing in the player changes state when only the setting does. Desktop is untouched: it never had a system equalizer, and its old side was empty stubs.
  - **The one deliberate departure from verbatim: CLOSE now carries `EXTRA_PACKAGE_NAME`.** AudioEffect's javadoc makes both extras mandatory for `ACTION_CLOSE_AUDIO_EFFECT_CONTROL_SESSION`, and AOSP MusicFX's `ControlPanelReceiver` returns early on a null package and closes sessions by package — so the restored CLOSE was silently dropped. Switching from SYSTEM back to BUILT_IN then left MusicFX attached while the built-in curve came back, both running until the track ended, and pausing could not fix it because by then the gate drops every request. With the extra, a pause now releases MusicFX's effects as documented; v1.7.0 kept them attached through pauses (MusicFX holds one session per package, so they never piled up). A stop issued mid-playback still ends re-attached, inherited and harmless under SYSTEM where the built-in curve is flat: `stop()` leaves `playWhenReady` true, and Media3 delivers `onEvents` through `sendMessageAtFrontOfQueue` — after the adapter's own CLOSE — so it computes `shouldBePlaying` and sends OPEN. Found in review, not on a device; closed-source EQ apps (SoundAlive, Wavelet, Poweramp EQ) are unverified.
  - **`onEvents` is the path that is easy to miss.** It calls `listeners.forEach { it.shouldOpenOrCloseEqualizerIntent(…) }` directly instead of going through `notifyEqualizerIntent`, so searching for the helper finds only five of the seven call sites — which is exactly how the first restore pass left it out.
  - **A gap carried over unchanged from the old code** (read from the code, not measured): a crossfade produces no event the adapter forwards — the incoming player starts playing before it is current, so its listener returns early, and the swap's `transitionToState(PLAYING)` is a no-op. Every ExoPlayer has its own audio session, so with crossfade on the system equalizer stays on the outgoing player's session until the promoted player next changes state (a rebuffer, a seek, a pause). Fixing it means announcing the new session on the swap, or giving all players one shared session.
- **Fullscreen lyrics becomes content + host, with a landscape layout (2026-09-13)**: the page is now `FullscreenLyricsContent` (new `ui/component/FullscreenLyricsContent.kt`), hosted the way Now Playing is. `FullscreenLyricsSheet` keeps its name but is only the Android host — the same `ModalBottomSheet` (container, scrim, insets, shape unchanged) around the content, animating its own hide when the page asks to close. On Desktop, `NowPlayingScreenContent` hosts the content in a `Popup` (`focusable = true`) instead, at `IntOffset.Zero` and sized to `LocalWindowInfo.containerSize` — the whole window, the 40dp custom title bar included, so while the page is open the bar's drag area and window buttons are under it. It first stopped below the bar, and a focusable Popup dismisses on any press outside itself, so a click on the bar closed the page; `PopupProperties(consumePointerInputOutside = false)` would let such clicks through, but it exists on skiko only (experimental, present in `ui-desktop` 1.12.0-alpha01) and is not part of commonMain's `expect` constructor. A Popup also appears in a single frame, which read as a flash over the whole window, so the page runs through `AnimatedVisibility(visibleState = MutableTransitionState(false).apply { targetState = true })` — fade plus scale from 0.96, 300ms in and 220ms out — and closing only flips the target: `showFullscreenLyrics` drops once the transition is idle at `false`, or the Popup would leave mid-exit. **The page clips itself to 12dp corners**: DesktopApp makes the window transparent and clips its content to `RoundedCornerShape(12.dp)` whenever it draws the custom title bar (both gated on `!isVM`), but a Popup is a layer of its own that clip never reaches, so a full-window page came out square-cornered. commonMain reads that condition as `containerSize.height > getScreenSizeInfo().hPX`, since `getScreenSizeInfo()` subtracts exactly that bar. The host block moved below `state`/`actions`, because the landscape layout takes the same contract the style content does — and the shell's own sheets (more, queue, info, add to playlist, vote) moved below the host: `orientation` is not in the activity's `configChanges`, so a rotation recreates it, every open sheet re-enters composition in one pass and attaches in declaration order, and the landscape page opens those same sheets on top of itself. On Android the sheet passes `sheetMaxWidth = Dp.Unspecified` in landscape only: Material 3 caps a `ModalBottomSheet` at 640dp by default, which would squeeze the two columns into a centred strip. Desktop has no drag or back gesture, so there the page draws its own chrome in both orientations: a glass pill with Close and the mini-player switch, and a glass pill with the mute toggle and a volume slider running the Desktop capsule's volume state machine from `MiniPlayer`.
  - **Portrait is the old page, moved verbatim.** Two mechanical adjustments only: the tap-to-show-controls `clickable` moved from the sheet onto the content root, and the root paints black itself — the sheet's `containerColor` used to sit under the gradient's translucent stops, and a Popup has no container. The artist tap now just calls `onDismiss()`; each host decides how it closes.
  - **Landscape is Apple Music's desktop full-screen player**: the left 45% holds the artwork (`min(maxWidth × 0.75, maxHeight × 0.5, 420dp)`) in a column 40dp wider than it — every style row carries 20dp side gutters, so that width lines the rows up with the artwork's edges — with the CURRENT Now Playing style's own track row and playback controls under it; the right 55% is the same `LyricsView` portrait shows. The background follows the style too: Apple Music → its frosted artwork, the other two → the animated gradient. Each style's action row (Info · Cast · Queue…) is deliberately left out. Orientation (`wDP > hDP`), not platform, picks the layout, so Android tablets and rotated phones get it as well; the left column scrolls, since a phone on its side cannot fit artwork, row and transport at once.
  - **Shared pieces extracted verbatim from the style files**: `NowPlayingTrackInfoRow` and `ExpressiveTrackInfoRow` (now `internal`, with `showCanvasThumbnail` so landscape drops the small canvas thumbnail beside a full-size artwork), `AppleMusicMainTitleRow` (`internal`), `SpotifyPlaybackControls` / `ExpressivePlaybackControls` / `AppleMusicPlaybackControls`, `NowPlayingExpressiveTheme` (M3E's artwork-seeded scheme), `AppleMusicArtworkBackdrop` (blurred artwork, `hqdefault` fallback, tint) and `AnimatedLyricsGradientBackground`. The three playback-controls functions are `ColumnScope` extensions that emit straight into the caller's Column instead of wrapping themselves: the Spotify and M3E panels pass `isElementVisible` in through `sliderModifier`, and that modifier compares the slider against its PARENT layout, so a wrapper would silently change what it measures against. `TimedLineIndex` / `activeIndexAt` became `internal` so the share picker could move with the page.
  - **Shuffle and repeat ride the transport row on this page only.** Now Playing keeps them elsewhere — M3 Expressive in `ExpressiveConnectedGroup`, Apple Music in its queue — and this page leaves both of those out, so `AppleMusicTransportRow` / `ExpressiveTransportRow` and their `*PlaybackControls` wrappers take `showShuffleAndRepeat = false` by default and only `FullscreenLyricsContent` passes `true`. Apple's row switches from its 58dp centred cluster to `SpaceBetween` when it carries five, since five at that gap no longer fit beside the artwork; M3 Expressive adds two narrower pills (`TOGGLE_WEIGHT`) in the connected group's on/off colours, set 24dp off the transport instead of the row's 8dp so they read as their own pair. Spotify's `PlayerControlLayout` already had both.

- **"More" on home sections, and a generic Browse page (2026-09-19)**: a home section that carries a `moreContentButton` now shows a More button, and its title opens the same place — measured on live responses, the title run's endpoint is identical to the button's (only `clickTrackingParams` differ), and a section without the button has no title endpoint either. The endpoint lands on `HomeItem.moreEndpoint` (`browseId`, `params`, `pageType`), read in `HomeParser`.
  - **Route by where the endpoint points, never by guessing from the id.** YouTube states the page type on the endpoint itself (`browseEndpointContextMusicConfig.pageType`): Explore's "Trending" More carries `MUSIC_PAGE_TYPE_PLAYLIST`. `navigateToMoreEndpoint` (in `AdapterItems.kt`) sends album/audiobook, playlist, artist/user-channel and podcast pages to their existing screens (`PlaylistRepositoryImpl` adds the `VL` prefix itself), and everything without a page type — the `FEmusic_*` ids: `listen_again`, `library_landing`, `moods_and_genres`, `new_releases_videos`… — to the new `BrowseDestination`. `params` travel verbatim, still URL-encoded (`ggMEgAKgAw%3D%3D`); ytmusicapi and Metrolist send them that way too.
  - **The Browse page copies Metrolist's `YouTubeBrowseScreen`**: `YouTube.browse()` in the scraper went from two shelf kinds to Metrolist's four — grid, carousel (two-row items, list rows and podcast episodes; immersive carousels arrive as carousels via `@JsonNames`), music shelf and playlist shelf — plus mood buttons (`musicNavigationButtonRenderer`), which is all `FEmusic_moods_and_genres` holds; each opens `MoodDestination(params)` from its own endpoint. Everything is flattened into one grid with `distinctBy { id }`, section titles dropped, and **no paging** — Metrolist's browse screens do not page either.
  - **Home's card dispatch is now `HomeContentCard`**, shared by Home's rows and the Browse grid, so an item opens and plays the same in both. It decides the card from which id is set and draws a wide thumbnail as a video card, so `YTItem.toHomeContent()` (in `Mapping.kt`) writes 544×544 for songs and 1280×720 for videos to say which.
  - **Justified rows, not a grid** — every thumbnail in a row is the same height, each card keeps its own shape (square, video 16:9), and the number of cards per row follows the shapes in it; `justifyRows()` in `BrowseScreen.kt` packs greedily toward a 180dp thumbnail and sizes each card to fill the row exactly (subtracting the cards' own 20dp padding, so the thumbnails — not the cards — come out equal). The shape comes from `Content.homeCardAspectRatio()`, which mirrors `HomeContentCard`'s dispatch. The cards take `fillMaxWidth` (default `false`, so Home/Library/Artist/Album are unchanged): `fillMaxWidth()` plus the card's own aspect ratio instead of the fixed 160dp / 284.5dp. Two grid attempts came first and both read as scattered: fixed-size cards in stretched `GridCells.Adaptive` cells with videos spanning two left a hole beside every card and a card-wide hole after every video; cards filling one cell each left videos squashed into a square cell's width. `LazyVerticalGrid` cannot vary the column count per row, which is the whole requirement once cards keep their own shapes.
  - It wears Mix for you's background: the artwork-tinted ambient glow (from the page's first item) and the bar that is transparent at the top and frosted once scrolled. The grid's top gap is `contentPadding`, not a spacer item, because the glow parks once item 0 has scrolled by — with a spacer as item 0 it parks after ~64dp, while most of the glow is still on screen.
  - **The two sections the app builds itself at the bottom of Home (issue #2461)** come from `FEmusic_new_releases` through `parseNewRelease`, not from `FEmusic_home`, so they needed their own wiring: `YouTube.newRelease()` now keeps each shelf's own More endpoint (`ExplorePage.releasedMoreEndpoint` from the first shelf, `musicVideoMoreEndpoint` from the last — the same shelves the two lists are read from), and `parseNewRelease` puts them on the two sections. The video one leads to `FEmusic_new_releases_videos`, a 100-item grid with no continuation, i.e. the whole list. **Signed out, the page has no album shelf** (measured 2026-09-19 in VN, US and GB, and `FEmusic_new_releases_albums` answers 404 there) — a signed-in session still gets one first, which is why "New releases" shows up for the owner. Measuring only anonymously led to the wrong conclusion that albums were gone; `parseNewRelease` now simply drops a section with no contents, which covers the signed-out case.
...
- **Radio queues trim their own history, and the per-append queue scan got cheap (2026-09-21, issue #2504)**: a radio queue keeps 100 played tracks behind the current one and drops the oldest once the history passes 120 (`RadioQueueTrim` in `core/domain`), so a listener can scroll back at most ~147 played tracks. Nothing ahead of the playing track is ever touched, and a playlist or album is trimmed only after *Endless queue* has carried it past the list the user picked — at which point `loadMore` re-points it at the last track's radio and now also re-types it `PlaylistType.RADIO` (in BOTH of its Endless branches), because from there on it IS a radio.
  - **`PlaylistType.RADIO` does not mean "a radio".** Favorites, Downloaded, Most played, Monthly recap, Your top tracks and the Analytics lists are queued as `RADIO` with `playlistId = null`, and a playlist's Shuffle is queued as `RADIO` with the playlist's own id. The first version gated on the type alone and trimmed all of them. `RadioQueueTrim.appliesTo` now requires the type AND a YouTube radio id: `RD…`, minus `RDCLAK…` (a curated playlist, which has an end).
  ...
  - **`MediaPlayerInterface.removeMediaItems(from, to, onRemoved)`** removes a range of played tracks in one pass (one precache top-up, one timeline notification). Removing one at a time would repeat exactly the storm above. The default refuses; only the two real adapters remove anything.
    - Both adapters re-check everything INSIDE their own launch (issue #2156's crash shape).
    - They refuse while `isCrossfading`: `crossfadeFromIndex` indexes the same playlist and is not shifted, so a cancelled fade would revert to a track ~100 positions away.
    - They refuse while shuffle is on: the tracks before the current one in the unshuffled list are then still to come.
    - Android also refuses while casting: `CastHandoffManager.remoteToPlaylist` holds absolute playlist positions, so shifting them sends the receiver the wrong tracks.
    - They do NOT call `clearPrecacheExceptCurrentInternal()`. That releases every precached handle, including the one the next crossfade is about to use; the removed tracks are all behind the playhead and precache is keyed by `mediaId`.
  - **`queueData` is cut by the removal's own answer, never by a guess.** The adapter calls `onRemoved(removedIds)` exactly once, on its own thread, from INSIDE the removal and before anything else can run. It passes an empty list when it refuses. The handler cuts `listTracks` right there, only if those ids are exactly its front (`RadioQueueTrim.afterFrontRemoved`). An `AtomicBoolean` in-flight flag stops a second request until the answer arrives.
    - The first version recorded an expected size and inferred the outcome from later timeline events, and **that broke Desktop outright**. `MpvPlayerAdapter.notifyListeners` QUEUES listener calls, each carrying a snapshot, so a batch's own add events arrived after the request and cleared it before the removal ran. The player was trimmed and `queueData` was not, for good.
    - With `queueData` longer than the player, the load-more check (`listTracks.size - player.currentMediaItemIndex < 3`) never fires again, so **the radio stopped** about 50 tracks later.
    - Two requests in flight at once (a batch plus a crossfade end, or a batch plus Add to queue) cut twice for the same reason.
    - Offset lists are the "tap a queue row, play the wrong song" bug, and `reorderShuffledQueue` bails on a size mismatch, so nothing heals them.
  ...
  - `TRIM_ABOVE_HISTORY` is 120, not the ~150 the history is meant to stay under: the check runs right after a batch append, and a radio appends roughly every 50th track, so a threshold is only crossed at ~47, ~97, ~147, ~197… At 150 the first cut waited until ~197; at 120 it lands at ~147 and the history swings 100…~147.
  - **With crossfade on, the batch lands mid-fade and the trim is refused** — the batch is requested by the transition that STARTS the fade and arrives 1–3 s later, inside a 5 s default fade. Without a retry, crossfade users would never be trimmed; the handlers therefore call `trimRadioHistoryIfNeeded()` again from `onCrossfadeStateChanged(false)`. That retry is **posted, never run inline**. Android calls `onCrossfadeStateChanged` from inside its own code: Next mid-fade commits the incoming track, then posts a seek to an index it has just computed. An inline trim therefore queued the removal ahead of that seek, and the seek landed ~47 tracks later. The fade remembers the track it may revert to as a playlist POSITION (`crossfadeFromIndex`), and an id would not do instead: a radio repeats songs, so ids are not unique in a queue.
  - **Anything that holds a queue POSITION over time must expect the queue to shrink from the front.** Before this, only the user's own actions shortened it.
    - `playNext` records just its insertion and applies it to the current list inside `_queueData.update`. It used to write back a copy of the whole queue taken before `getSongInfo` suspended, which silently undid a trim that landed meanwhile.
    - The handlers' `removeMediaItem`/`moveItemUp`/`moveItemDown` check the position against the current queue and edit it inside `update`.
    - The queue ⋯ sheet, on both queue screens, remembers which track it was opened for and does nothing once that track is no longer at its position.
    - Android's `removeMediaItem`/`moveMediaItem`/`replaceMediaItem` and Desktop's `replaceMediaItem` re-check bounds inside their launch (#2156's crash shape).
    - `reorderShuffledQueue` writes only over the exact list it read (`it.data.listTracks === listTrack`). On Desktop, Add to queue runs it on the UI thread right after requesting a trim, while the cut lands on the player thread; writing its snapshot afterwards put the trimmed tracks back.
  - Queue rows (the Classic/M3E sheet and the Apple Music queue) are keyed `videoId#occurrence` instead of `index + videoId`: a front trim changes every position, so a position-based key made the whole list lose its scroll anchor and jump under the user.
  - Now Playing's artwork pager snaps instead of animating when the index moves more than one page — a trim drops it by ~47 without changing the track, and animating that flings the pager through dozens of covers.

- **Login sync: sign-ins from phone to desktop over the LAN (2026-09-21)**: Settings → Content gains a row that reads *Send sign-ins to desktop* on Android and *Sign in from your phone* on Desktop. Desktop shows a QR (`simpmusic://login-sync?h=<ips>&p=<port>&k=<key>`) and runs a one-shot **Ktor server (CIO engine)** only while that dialog is open — 5 minutes, restarted when the phone connects. Android scans it (CameraX + ZXing), sends a sealed hello, gets the computer's name back sealed under the same key, asks *Trust <name> (<os>)?*, then lets the user tick services and sends only those. Layered like Listen Together: **service** `core/service/loginSync` owns the transport and nothing else — invite/QR, AES-GCM, `LoginSyncClient` (commonMain, `ktorExt`'s engine) and `LoginSyncServer` + `reachableAddresses()` (jvmMain, `ktor-server-cio`); it moves one opaque payload. **domain** holds `LoginSyncHostRepository` (Desktop) and `LoginSyncSenderRepository` (Android) plus `model/loginsync` types. **data** holds what the payload means — `LoginBundle` and `LoginSyncStore` in `commonMain` — and the two impls in `jvmMain` / `androidMain`, each bound by `loadLoginSyncModule()` on its own platform (iOS binds nothing); core/data itself imports no Ktor. **composeApp** holds only `LoginSyncHostViewModel` / `LoginSyncSenderViewModel` and the dialog/sheet. Two repository interfaces rather than one so no platform carries methods it cannot implement; both VMs are declared in the common Koin module, which is safe because Koin resolves a dependency when the VM is created, and each platform only ever creates its own.
  - **Only sign-ins travel, by allowlist**: YouTube (every `GoogleAccountEntity` row — each carries its own cookie in `cache` and `netscapeCookie` — plus the current `cookie`/`page_id`/`auth_user`/`data_sync_id`/`logged_in`), Spotify `sp_dc`, the Discord token, the Last.fm session. No settings, no AI keys, no proxy credentials. The picker lists service names only, never account names.
  - **Room before DataStore on restore.** `CommonRepositoryImpl` rewrites `ytdlp-cookie.txt` from the *used* account the moment the DataStore cookie changes, so writing the cookie first writes the previous account's file with no error anywhere. A new `sp_dc` also clears Spotify's personal token, as the logout does.
  - **The key travels on the screen, never over the network.** Every body is AES-256-GCM (`nonce || ciphertext || tag`) with `simpmusic-login-sync/v1/<kind>/<request|reply>` as associated data, so a hello cannot be replayed as a login bundle and a request cannot pass as a reply. The name coming back sealed is what proves the machine answering is the one showing the QR.
  - **composeApp's `commonMain` can use the JDK** (it targets only Android and JVM — `Platform.kt` already calls `System.getProperty`), and the loginSync service targets only Android + JVM for the same reason, so its cipher is plain javax.crypto in commonMain. Its server handlers are suspend functions, so the restore is a plain call. `core/service/loginSync/src/jvmTest` has a real client↔server round trip on 127.0.0.1 — hello, payload, reply, then the spent code refused. `stop()` / `disconnect()` are deliberately NOT suspend: callers are teardown paths whose scope is already being cancelled, so the repository runs them on its own scope, and the server stop is `NonCancellable` so a cancelled caller cannot leave the port open. **Do not go back to JDK `com.sun.net.httpserver`**: with its default executor every handler runs on the dispatcher thread, and `HttpServer.stop()` joins that thread — the first version called it from `onDispose` on the UI thread while a restore was in flight, and the desktop app froze.
  - **CameraX + ZXing, not ML Kit or Google's code scanner** — both are proprietary and F-Droid refuses them. `CAMERA` is paired with `uses-feature android.hardware.camera required="false"`; without it the permission implies a required camera and hides the app from TVs and cars.
  - **Android UI is a bottom sheet** in the app's own sheet style (transparent `ModalBottomSheet` + `Card` on `rememberSurfaceDarkColors()`): header with a close button, a square rounded viewfinder, and in-sheet states for camera permission — ask, and once denied with "don't ask again" (`shouldShowRequestPermissionRationale` false *after* a denial) an *Open settings* button, re-checked on resume. Trust and service picking stay popups over the sheet. **`PreviewView` must use `ImplementationMode.COMPATIBLE`**: the default SurfaceView ignores Compose clipping, so the rounded viewfinder renders with square corners.
  - **Android 17 local network permission**: `ACCESS_LOCAL_NETWORK` is declared, and requested at runtime only once targetSdk is 37+ — without it a LAN connection does not fail, it times out (element-x-android#7694). targetSdk is 36 today, so nothing is asked yet.
  - **Addresses are not limited to private ranges or to Wi-Fi.** Every up, non-loopback, non-link-local IPv4 is listed under the QR as a radio list, in interface order with no ranking, and the QR carries only the one the user picks; switching re-points the QR without restarting the server, which listens on every interface anyway. `isSiteLocalAddress` alone reported "no network" on a corporate LAN numbered from 21.x, and excluding point-to-point interfaces threw away the Tailscale address (100.x on `tailscale0`) — which is what lets a phone on the same tailnet pair from anywhere. Only container/hypervisor bridges (`docker`, `br-`, `veth`, `virbr`, `vboxnet`, `vmnet`, `podman`, `cni`) are dropped, since their addresses exist only inside the machine.
...
- **Now Playing video frames take the video's shape (2026-09-25)**: the frame around an inline video was a hard 16:9 in all three styles, so a 4:3 video sat pillarboxed and a vertical one shrank to a sliver. Every style now reads one value, `NowPlayingContentState.videoAspectRatio` (16:9 until known), and fits the frame into the space that style already reserves for artwork — never more, so the one-screen layout cannot grow. Spotify fits it inside the square slot; the hidden thumbnail under it takes the same shape while the video plays, because its box is what casts the artwork shadow — left at 16:9, that shadow ringed a tall video. M3E's card takes the video's shape inside a slot capped at square, and `expressiveCardSlotRatio()` is the ONE expression both the card and its measuring spacer use. Apple Music fits it into the zone above the controls, starting 16dp below the top chrome (status bar + 28dp grabber on Android, the glass dismiss button on Desktop), because the frame's top-right corner carries the fullscreen button. That zone is the page's own height minus the controls, NOT `artworkZoneHeightDp`: that value is measured against the window, and the Desktop side panel is shorter than the window by its shell padding, so a tall frame ran into the gap above the title.
  - The value comes from `rememberVideoAspectRatio(playerName)` (expect/actual). Android reads Media3's `rememberPresentationState(player).videoSizeDp`. Desktop reads mpv's `video-params/aspect`, observed by `MpvPlayer` and published as `MpvVideoFrameSource.aspectRatio` — the frames cannot say it themselves, since mpv letterboxes them to the target size. Living on the frame source, it is re-pointed by `MpvPlayerAdapter.currentVideoFrames` on every handle swap for free.
  - **The Full build's session player only reports what changed.** There `MAIN_PLAYER` is a Media3 `CastPlayer` (`ForwardingSimpleBasePlayer` → `SimpleBasePlayer`): it re-reads the wrapped player on `onEvents` and fires a callback only for fields that differ from its last snapshot. A view composed mid-video — Now Playing reopened after fullscreen — therefore never hears `onVideoSizeChanged`, `onTracksChanged` or `onMediaItemTransition` for the current item. `MediaPlayerViewWithSubtitle` started at `videoRatio = 16f / 9` and waited for that event, so the video came back stretched to 16:9 ever since Cast wrapping landed (core `c310577`, 2026-07-10); FOSS passes events straight through and healed on the next frame. UI state derived from the session player must be read from the player's getters (or Media3's `rememberPresentationState`) before listening.
